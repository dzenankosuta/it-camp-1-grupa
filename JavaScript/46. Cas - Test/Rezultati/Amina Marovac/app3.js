...
const first = CART_DATA.map((element) => element.amount).reduce(
  (a, b) => a + b
);

const firstt = CART_DATA.map((element) => {
  return {
    name: element.name,
    total: element.price * element.amount,
  };
});
const potr = firstt.map((element) => element.total);
const shippingCosts = 320;
const second = potr.reduce((a, b) => a + b, shippingCosts);
return ``;
